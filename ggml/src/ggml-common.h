...
// Storage and transform group are both 128 (head_dim, for rotation Gaussianization)
...
// = 66 bytes per 128 values = 4.125 bits/value → 3.88× compression vs fp16
typedef struct {
...
// Note: asking for a turbo2 V cache does not give one throughout. Layer-adaptive
// Boundary V turns itself on (see llama_kv_cache) and puts q8_0 on the boundary
// layers, so the V cache measures larger than this block implies.
